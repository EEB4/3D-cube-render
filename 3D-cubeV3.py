...
while True:
    ...
    #creating array for all vertice positions in 2d space
    coords = []
    #creating array for all z-coordinates of points (disance from camera)
    zdist = []
    ...
    for point in points:
        ...
        #adding the 2d coordinates of each point into the coord array to then use for making faces
        coords.append((x, y))
        zdist.append(float(projected2d[2][0]))
    ...
    #adding 3d faces by setting coordinates of polygon vertices as 2d coordinates of the points in the cube
    #if the position of the center of face 1 is infront
    if (zdist[0]+zdist[1]+zdist[2]+zdist[3])/4 >= (zdist[4]+zdist[5]+zdist[6]+zdist[7])/4:
        pg.draw.polygon(screen, (0, 255, 0), [coords[4], coords[5], coords[6], coords[7]])
    else:
        pg.draw.polygon(screen, (255, 0, 0), [coords[0], coords[1], coords[2], coords[3]])
